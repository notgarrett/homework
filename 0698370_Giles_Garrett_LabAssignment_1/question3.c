...
bool palendromeCompare(STACK *, char *);

void pushLoop(STACK *, char *);

int main(void) {
  STACK *pStackTop;
  pStackTop = createStack();
  char palendrome[] = "set tes";
  pushLoop(pStackTop, palendrome);

  if (palendromeCompare(pStackTop, palendrome))
    printf("%s is a palendrome", palendrome);
  else
    printf("%s is not a palendrome", palendrome);
}

bool palendromeCompare(STACK *pStackTop, char *palendrome) {
  if (*palendrome == '\0')
    return 1;
  char *dataptr = (char *)malloc(sizeof(char));
  dataptr = stackTop(pStackTop);
  if (*palendrome == *dataptr) {
    popStack(pStackTop);
    return palendromeCompare(pStackTop, ++palendrome);
  }
  return 0;
}

void pushLoop(STACK *pStackTop, char *string) {
  // Simple recursion that pushes a string into the stack.
  if (*string == '\0')
    return;
  char *dataptr = (char *)malloc(sizeof(char));
  *dataptr = *string;
  char *stringptr = (char *)malloc(sizeof(char));
  stringptr = stackTop(pStackTop);
  pushStack(pStackTop, dataptr);
